...
alldata <- genderdata %>%
            inner_join(select(abcd_y_lt,c(src_subject_id,eventname,
                                          site_id_l,interview_age)),
                          by=c("src_subject_id","eventname")) %>%
            rename(age = interview_age, site = site_id_l) %>%
            # inner_join(erqdata,by=c("src_subject_id","eventname")) %>%
            inner_join(dersdata,by=c("src_subject_id","eventname")) %>%
            inner_join(ledata,by=c("src_subject_id","eventname")) %>%
            inner_join(cbcldata,by=c("src_subject_id","eventname")) %>%
...
yr4data %>% summary()
yr4data %>% group_by(genderid) %>% summarize()
...
yr4data %>% select(c(total_bad_le,ders6total,cbcl_total,cbcl_int,cbcl_ext)) %>% corr.test()
...
ggplot(df_proportions, aes(x=total_bad_le,y=proportion)) +
...
ggplot(yr4data, aes(x = ders6total,fill=genderid)) +
  ...
  theme_classic()
df_proportions <- yr4data %>%
  mutate(ders6total_bin = cut(ders6total, breaks = seq(0, max(ders6total)+10, by = 10), right = FALSE)) %>% # Create bins
...
ggplot(df_proportions, aes(x = ders6total_bin,y=proportion,fill=genderid)) +
  geom_col_pattern(aes(
          ...
          scale_y_continuous(expand = c(0,0),breaks=seq(min(df_proportions$proportion),
                                                        ...
                                                        by=0.05)) +
            theme_classic()
# ggsave("SuppFig1B.tiff",width=7,height=3,unit="in")
  # geom_bar(stat="identity",position="dodge") + 
  # scale_x_discrete(labels=c("0-9","10-19","20-29","30-39","40-49","50-59","60-69",
  #                             "70-79","80-89","90-99","100-109","110-119","120-129",
  #                             "130-139")) +
...
# ggplot(df_proportions, aes(x = ders6total_bin,y=proportion,fill=genderid)) +
  ...
  #                                               by=0.05)) +
  theme_classic()
# ggsave("SuppFig1C.tiff",width=4.5,height=3,unit="in")
yr4data %>%
  summarise(
...
#   #                                               by=0.05)) +
#   theme_classic()
# ggsave("SuppFig1D.tiff",width=4.5,height=3,unit="in")
# geom_bar(stat="identity",position="dodge") + 
#   scale_x_discrete(labels=c("0-9","10-19","20-29","30-39","40-49","50-59","60-69",
#                             "70-79","80-89","90-99")) +
#   scale_y_continuous(expand = c(0,0),breaks=seq(min(df_proportions$proportion),
...
ggplot(df_proportions, aes(x = cbcl_ext_bin,y=proportion,fill=genderid)) +
  ...
  #                                               by=0.05)) +
  theme_classic()
ggsave("SuppFig1E.tiff",width=4.5,height=3,unit="in")
# geom_bar(stat="identity",position="dodge") + 
#   scale_x_discrete(labels=c("0-9","10-19","20-29","30-39","40-49","50-59","60-69",
#                             "70-79","80-89","90-99")) +
#   scale_y_continuous(expand = c(0,0),breaks=seq(min(df_proportions$proportion),
...
### mediation models with mediation package ####
### note this doesn't look like it works with mixed effect models
X <- yr4data$total_bad_le
M <- yr4data$ders6total
Y<- yr4data$cbcl_total
age <- yr4data$age
site <- yr4data$site
testdata <- data.frame(X = X, M = M, Y = Y, age = age, site = site)
model.M <- lm(M ~ X + age, testdata)
# model.M <- lmer(M ~ X + age,data=testdata)
summary(model.M)
model.Y <- lm(Y ~ X + M + age, testdata)
# model.Y <- lmer(Y ~ X + M + age,data=testdata)
summary(model.Y)
library(mediation)
...
meddata <- yr4data %>% 
              select(total_bad_le_CMC,ders6total_CMC,
                     cbcl_total_CMC,cbcl_int_CMC,cbcl_ext_CMC,
                     log_total_bad_le_CMC,logders6total_CMC,logcbcl_total_CMC,
                     logcbcl_ext_CMC,logcbcl_int_CMC,
                     age,site,genderid,sex) %>%
              rename(LES = total_bad_le_CMC,
                     DERS = ders6total_CMC,
                     totalCBCL = cbcl_total_CMC,
                     intCBCL = cbcl_int_CMC,
                     extCBCL = cbcl_ext_CMC,
                     logLES = log_total_bad_le_CMC,
                     logDERS = logders6total_CMC,
                     logtotalCBCL = logcbcl_total_CMC,
                     logintCBCL = logcbcl_int_CMC,
                     logextCBCL = logcbcl_ext_CMC) %>%
...
gendertotalmodel <- ' # direct effect 
             totalCBCL~ c("c1","c2","c3")*LES + c("m1","m2","m3")*age
           # mediator 
             DERS ~ c("a1","a2","a3")*LES  + c("n1","n2","n3")*age
           # indirect effect 
...
gender_constall_totalmodel <- ' # direct effect 
             totalCBCL~ c("c1","c1","c1")*LES + c("m1","m1","m1")*age
           # mediator 
             DERS ~ c("a1","a1","a1")*LES  + c("n1","n1","n1")*age
           # indirect effect 
...
sextotalmodel <- ' # direct effect
             totalCBCL~ c("c1","c2")*LES + c("m1","m2")*age
           # mediator
             DERS ~ c("a1","a2")*LES  + c("n1","n2")*age
           # indirect effect
...
sex_constall_totalmodel <- ' # direct effect 
             totalCBCL~ c("c1","c1")*LES + c("m1","m1")*age
           # mediator 
             DERS ~ c("a1","a1")*LES  + c("n1","n1")*age
           # indirect effect 
...
genderintmodel <- ' # direct effect 
             intCBCL~ c("c1","c2","c3")*LES + c("m1","m2","m3")*age
           # mediator 
             DERS ~ c("a1","a2","a3")*LES  + c("n1","n2","n3")*age
           # indirect effect 
...
gender_constall_intmodel <- ' # direct effect 
             intCBCL~ c("c1","c1","c1")*LES + c("m1","m1","m1")*age
           # mediator 
             DERS ~ c("a1","a1","a1")*LES  + c("n1","n1","n1")*age
           # indirect effect 
...
sexintmodel <- ' # direct effect
             intCBCL~ c("c1","c2")*LES + c("m1","m2")*age
           # mediator
             DERS ~ c("a1","a2")*LES  + c("n1","n2")*age
           # indirect effect
...
sex_constall_intmodel <- ' # direct effect 
             intCBCL~ c("c1","c1")*LES + c("m1","m1")*age
           # mediator 
             DERS ~ c("a1","a1")*LES  + c("n1","n1")*age
           # indirect effect 
...
genderextmodel <- ' # direct effect 
             extCBCL~ c("c1","c2","c3")*LES + c("m1","m2","m3")*age
           # mediator 
             DERS ~ c("a1","a2","a3")*LES  + c("n1","n2","n3")*age
           # indirect effect 
...
gender_constall_extmodel <- ' # direct effect 
             extCBCL~ c("c1","c1","c1")*LES + c("m1","m1","m1")*age
           # mediator 
             DERS ~ c("a1","a1","a1")*LES  + c("n1","n1","n1")*age
           # indirect effect 
...
sexextmodel <- ' # direct effect
             extCBCL~ c("c1","c2")*LES + c("m1","m2")*age
           # mediator
             DERS ~ c("a1","a2")*LES  + c("n1","n2")*age
           # indirect effect
...
sex_constall_extmodel <- ' # direct effect 
             extCBCL~ c("c1","c1")*LES + c("m1","m1")*age
           # mediator 
             DERS ~ c("a1","a1")*LES  + c("n1","n1")*age
           # indirect effect 
